...
//            }
            ...
            googleMap.addMarker(MarkerOptions().position(location1).title("Ahmedabad").icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_BLUE)))
            googleMap.animateCamera(CameraUpdateFactory.newLatLngZoom(location1,10f))

            val location2 = LatLng(23.21,72.63)
            googleMap.addMarker(MarkerOptions().position(location2).title("Gandhinagar"))

            val location3 = LatLng(23.03,72.46)
            googleMap.addMarker(MarkerOptions().position(location3).title("Bopal"))
        })
    }
    ...
    private fun checkPermissionsAndCurrentLocationAccess() {
        ...
        if (permissionGranted) {
            ...
            if (ActivityCompat.shouldShowRequestPermissionRationale(this, Manifest.permission.ACCESS_FINE_LOCATION)) {
                ...
                cameraPermissionAlertDialog.setMessage(getString(R.string.must_allow_location))
